...
public class UserController {
   ...
   * @throws AppException if the username exists ({@code USER_EXISTED}), a role does not exist
   *     ({@code ROLE_NOT_FOUND}), or restricted-role assignment is forbidden ({@code
   *     FORBIDDEN_ASSIGN_ROLE})
   */
  @PostMapping
  public ApiResponse<UserSummaryResponse> createUser(
   ...
   * @throws AppException if the update is empty ({@code INVALID_USER_UPDATE}), the user is not
   *     visible or does not exist ({@code USER_NOT_FOUND}), a role does not exist ({@code
   ...
   * @throws AppException if the user does not exist ({@code USER_NOT_FOUND}), the old password is
   *     incorrect ({@code INCORRECT_OLD_PASSWORD}), or the new password matches it ({@code
