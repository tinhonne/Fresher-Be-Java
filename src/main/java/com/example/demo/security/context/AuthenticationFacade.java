...
public interface AuthenticationFacade {
  ...
  /**
   * Determines whether the current authentication has the specified role. The {@code ROLE_} prefix
   * is added when it is not already present.
   *
   * @param role the role name
   * @return {@code true} when the role is granted
   */
  boolean hasRole(AppRole role);
