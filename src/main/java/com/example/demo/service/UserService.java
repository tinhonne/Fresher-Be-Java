...
public interface UserService {
   ...
   * @throws AppException if the username exists ({@code USER_EXISTED}), the default or requested
   *     role does not exist ({@code ROLE_NOT_FOUND}), or assignment of a restricted role is
   *     forbidden ({@code FORBIDDEN_ASSIGN_ROLE})
   */
   ...
   * @throws AppException if the authenticated user is neither an administrator nor a manager
   *     ({@code FORBIDDEN})
   */
   ...
   * @throws AppException if the user is not visible or does not exist ({@code USER_NOT_FOUND}), or
   *     access is forbidden ({@code FORBIDDEN})
   */
   ...
   * @throws AppException if no update is supplied ({@code INVALID_USER_UPDATE}), the user is not
   *     visible or does not exist ({@code USER_NOT_FOUND}), requested roles do not exist ({@code
   *     ROLE_NOT_FOUND}), or the update is forbidden ({@code FORBIDDEN})
   */
   ...
   * @throws AppException if the authenticated user does not exist ({@code USER_NOT_FOUND}), the old
   *     password is incorrect ({@code INCORRECT_OLD_PASSWORD}), or the new password matches the old
   *     password ({@code NEW_PASSWORD_SAME_AS_OLD})
   */
