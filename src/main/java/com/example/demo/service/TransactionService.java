...
public interface TransactionService {
     ...
     * @throws AppException if the amount is invalid ({@code INVALID_TRANSFER_AMOUNT}), the accounts are identical ({@code SAME_ACCOUNT_TRANSFER}), the source or destination does not exist ({@code SOURCE_ACCOUNT_NOT_FOUND} or {@code DESTINATION_ACCOUNT_NOT_FOUND}), or either account is inactive ({@code SOURCE_ACCOUNT_INACTIVE} or {@code DESTINATION_ACCOUNT_INACTIVE})
     */
     ...
     * @throws AppException if the account does not exist ({@code ACCOUNT_NOT_FOUND}), the date range is invalid ({@code INVALID_TRANSACTION_DATE_RANGE}), or pagination is invalid ({@code INVALID_PAGE_REQUEST})
     */
