...
public interface AccountRepository extends JpaRepository<Account, Long> {
  ...
  /**
   * Finds an account by number and acquires a pessimistic write lock for the surrounding
   * transaction.
   *
  ...
  boolean existsByCustomerIdAndStatusIn(
      Long id, Iterable<com.example.demo.entity.AccountStatus> statuses);

  @Query(
      "select a from Account a where a.customer.id = :customerId order by a.accountNumber asc, a.id asc")
  Page<Account> findByCustomerIdOrderByAccountNumber(Long customerId, Pageable pageable);

  @Query(
      "select a from Account a where a.customer.id = :customerId and a.status = :status order by a.accountNumber asc, a.id asc")
  Page<Account> findByCustomerIdAndStatusOrderByAccountNumber(
      Long customerId, com.example.demo.entity.AccountStatus status, Pageable pageable);
