...
public class ApiResponse<T> {
  ...
  /**
   * Creates a successful API envelope using the configured success code and message. The result is
   * preserved as supplied and may be {@code null}.
   *
   ...
   */
  public static ApiResponse<?> error(ErrorDefinition errorCode) {
    return ApiResponse.builder().code(errorCode.getCode()).message(errorCode.getMessage()).build();
  }

  /**
   * Creates an error envelope using an error code's numeric code and a replacement message, leaving
   * the result unset.
   *
   ...
   * @return an error response envelope
   */
  public static ApiResponse<?> error(ErrorDefinition errorCode, String message) {
    return ApiResponse.builder().code(errorCode.getCode()).message(message).build();
  }
}
