...
public class CustomerSpecification {
  ...
  /**
   * Matches customers whose names contain the supplied value, ignoring case. A null or blank value
  ...
  /**
   * Matches customers whose identity numbers contain the supplied value, ignoring case. A null or
   * blank value does not restrict the query.
   *
  ...
  /**
   * Matches customers whose mobile numbers contain the supplied value, ignoring case. A null or
   * blank value does not restrict the query.
   *
  ...
  private static Specification<Customer> hasCustomerType(CustomerType customerType) {
    return (root, query, cb) ->
        customerType == null ? cb.conjunction() : cb.equal(root.get("customerType"), customerType);
  }

  /**
   * Combines the request's optional filters with logical AND. Text filters use case-insensitive
   * substring matching; status and customer type use exact matching. Null or blank individual
   * values do not restrict the query.
   *
