...
class AccountControllerTest {
    ...
    void canonicalAndCompatibilityNumberRoutesResolve() throws Exception {
        when(accountService.getAccountByAccountNumber("1234567890123")).thenReturn(response("1234567890123"));

        mockMvc.perform(get("/accounts/number/1234567890123"))
                .andExpect(status().isOk());
        mockMvc.perform(get("/accounts/by-number/1234567890123"))
                .andExpect(status().isOk());
    }
    ...
    void listUsesDefaultsAndExplicitPagination() throws Exception {
        ...
        mockMvc.perform(get("/accounts").param("page", "2").param("size", "25"))
                .andExpect(status().isOk())
    ...
    void canonicalCustomerActiveRouteAndCompatibilityRouteResolve() throws Exception {
        ...
        mockMvc.perform(get("/customers/7/accounts/active"))
                .andExpect(status().isOk());
        mockMvc.perform(get("/accounts/7/active").param("size", "10"))
                .andExpect(status().isOk());
    }

    @Test
    void malformedIdDoesNotInvokeService() throws Exception {
        mockMvc.perform(get("/accounts/not-a-number"))
                .andExpect(status().isBadRequest());
